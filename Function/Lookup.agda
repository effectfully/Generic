...
Rec C (var i)        x      = C x
Rec C (π {{q}} b P)  f      = Coerce′ q (∃ λ x -> Rec C (unproj₂ P x) (apply b (uncoerce′ q f) x))
Rec C (D ⊛ E)       (x , y) = Rec C D x ⊎ Rec C E y

There : ∀ {ι β s} {I : Set ι} {B : I -> Set β} {j}
      -> (∀ {i} -> B i -> Set β) -> (D : Desc I β s) -> Extend D B j -> Set β
There C (var i)        e      = ⊥
There C (π {{q}} b P)  p      = split′ q p (There C ∘ unproj₂ P)
...
glookupTo : ∀ {ι α β s} {A : Set α} {I : Set ι} {D : Desc I β s} {B j} {e : Extend D B j}
          -> To A D e -> A
glookupTo (hereπ {{q}} {p = p}) = unproj₁′ q p
...
mutual
  ...
  glookupRec (π {{q}} b P)  p       = let x , r = uncoerce′ q p in glookupRec (unproj₂ P x) r
  ...
  glookupThere (π {{q}} b P)  p       t       =
    let x , e = uncoerce′ q p in glookupThere (unproj₂ P x) e t
